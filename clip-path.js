...
function resolveVertexPoints(val, toks, boxW, boxH) {
  ...
  if (v.startsWith('polygon(')) {
    ...
    while ((cm = cmdRe.exec(pathStr)) !== null) {
      ...
      // currentValue is always in 260-space; dots must be % of that design size
      const toBoxPct = (n, _isY) => (n / PATH_DESIGN_SIZE) * 100;
      if (cmd === 'M' || cmd === 'L') {
      ...
      const startDrag = (clientX, clientY) => {
        ...
        const onMove = (mx, my) => {
          ...
          pt.tokenIndices.forEach((ti, k) => {
            const tok   = tokens[ti];
            const isY   = k === 1 || (pt.tokenIndices.length === 1 && pt._axis === 'y');
            const delta = isY ? dy : dx;
