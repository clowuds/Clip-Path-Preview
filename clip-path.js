...
function resolveVertexPoints(val, toks, boxW, boxH) {
  ...
  if (v.startsWith('polygon(')) {
    ...
    points.push({ x: l, y: t, tokenIndices: [0, 3] }); // top-left (top+left)
    points.push({ x: r, y: t, tokenIndices: [0, 1] }); // top-right (top+right)
    points.push({ x: r, y: b, tokenIndices: [1, 2] }); // bottom-right
    points.push({ x: l, y: b, tokenIndices: [2, 3] }); // bottom-left
  }
    ...
    while ((cm = cmdRe.exec(pathStr)) !== null) {
      ...
      const toBoxPct = (n, isY) => (n / (isY ? boxH : boxW)) * 100;
      if (cmd === 'M' || cmd === 'L') {
      ...
      } else if (cmd === 'H') { nums.forEach(n => { cx = n; points.push({ x: toBoxPct(cx, false), y: toBoxPct(cy, true), tokenIndices: [] }); }); }
      else if (cmd === 'V') { nums.forEach(n => { cy = n; points.push({ x: toBoxPct(cx, false), y: toBoxPct(cy, true), tokenIndices: [] }); }); }
      else if (cmd === 'C') {
...
// When a path() is active we lock the box to that size so the shape always fits
// correctly, regardless of what the user last typed in the W/H inputs.
// When switching away from path() we restore whatever size was set before.
let _sizeBeforePath = null; // { w, h } saved when entering path() mode

function apply(val, fromSlider = false) {
  const isPath = /^\s*path\s*\(/i.test(val);

  if (isPath) {
    // Save current size the first time we enter path() mode
    if (_sizeBeforePath === null) {
      _sizeBeforePath = { w: box.offsetWidth || 260, h: box.offsetHeight || 260 };
    }
    // Force box to 260×260 — the coordinate space all path() presets use
    ...
    wInput.title = 'Resize disabled — path() uses absolute pixel coordinates';
    hInput.title = 'Resize disabled — path() uses absolute pixel coordinates';
  } else {
    // Leaving path() mode — restore previous size
    if (_sizeBeforePath !== null) {
      box.style.width  = _sizeBeforePath.w + 'px';
      box.style.height = _sizeBeforePath.h + 'px';
      wInput.value = _sizeBeforePath.w;
      hInput.value = _sizeBeforePath.h;
      _sizeBeforePath = null;
    }
  ...
  tokens.forEach((tok, i) => {
    ...
    block.innerHTML = `
      <div class="slider-top">
        ...
        <span class="slider-val-display" id="val-${i}">${tok.num}${tok.unit}</span>
...
// ── Size ──────────────────────────────────────────────────────────────────────
wInput.addEventListener('input', () => { if (!wInput.disabled) box.style.width  = wInput.value + 'px'; });
hInput.addEventListener('input', () => { if (!hInput.disabled) box.style.height = hInput.value + 'px'; });
