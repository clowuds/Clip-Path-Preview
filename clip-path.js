...
// ── Size ──────────────────────────────────────────────────────────────────────
const boxWrap = document.getElementById('preview-box-wrap');

function updateBoxDimensions() {
  const w = wInput.value || 260;
  const h = hInput.value || 260;
  
  // Pass input values to CSS custom properties
  boxWrap.style.setProperty('--box-w', w);
  boxWrap.style.setProperty('--box-h', h);
  
  // Re-apply path scaling and refresh vertex overlay dots
  apply(currentValue, true);
  renderOverlay(-1);
}

wInput.addEventListener('input', updateBoxDimensions);
hInput.addEventListener('input', updateBoxDimensions);

// Initialize box dimensions on load
updateBoxDimensions();
...
// ── Handle Screen / Window Resize ─────────────────────────────────────────────
const resizeObserver = new ResizeObserver(() => {
  // Recalculates path() scaling and updates vertex handle positions instantly
  apply(currentValue, true);
  renderOverlay(-1);
});

// Watch the preview box for any rendered size changes
